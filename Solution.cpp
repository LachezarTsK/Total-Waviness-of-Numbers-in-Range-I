...
class Solution {
    ...
    static const int MIN_WAVY_VALUE = 101;

public:
    int totalWaviness(int lowerLimit, int upperLimit) {
        int totalWaviness = 0;
...
private:
    bool isWavy(int first, int second, int third) {
        return (first < second && third < second) || (first > second && third > second);
    }

    int findWavinessOfValue(int value) {
        int waviness = 0;
